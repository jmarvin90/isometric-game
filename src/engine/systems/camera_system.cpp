    ...
    const SDL_DisplayMode& display_mode)
{
    ...
    if (
        mouse.window_current_position.x < constants::SCENE_BORDER_PX.x && 
        camera.camera_rect.x > 0
    ) {
        camera.camera_rect.x -= constants::CAMERA_SCROLL_SPEED.x;
    }

    if (
        mouse.window_current_position.y < constants::SCENE_BORDER_PX.y && 
        camera.camera_rect.y > 0
    ) {
        camera.camera_rect.y -= constants::CAMERA_SCROLL_SPEED.y;
    }

    if (
        (display_mode.w - mouse.window_current_position.x) < constants::SCENE_BORDER_PX.x && 
    ...
    if (
        (display_mode.h - mouse.window_current_position.y) < constants::SCENE_BORDER_PX.y && 
